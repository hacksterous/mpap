...
class mpap ():
	...
	def __truediv__ (self, other):
		...
		while (len(opResult) < (PREC+1) and opSelf != 0):
			#print ("len(opResult) = ", len(opResult), " -- opSelf = ", opSelf)
			opSelf = opSelf * 10
			bEPrecision += 1
		...
		#print ("truediv: returning with: ", bDiv, " and ", eDiv)
	...
	def asin (self, acosine=False):
		...
		if abs (self - 1) < mpap(1, -self.Precision):
			...
			u = mpap(1) - self * self
			if abs(u) < mpap(1, -self.Precision):
				if acosine == False:
					v = mpap(0)
				else:
					v = mpap(0.5).pi()
	...
	def asinalt (self, acosine=False):
		global MPAP_DEGREES_MODE
		global MPAPERRORFLAG
		if abs(self) > 1:
			MPAPERRORFLAG = "Domain error."
		...
		n = 0
		old = mpap(0)
		while abs(fact) > mpap(1, -self.Precision):
			#calculate term(n + 1) as per the formulas above
			nA = (n*2 + 1) * (n*2 + 2) * (n*2 + 1)
			nB = 4 * (n + 1)*(n + 1) * (n*2 + 3)
			fact = fact * mpap(nA)
			fact = fact / mpap(nB)
			fact = fact * self * self
			...
			n += 1
			old = res
			#print ("asin: nA/nB = ", nA/nB)
			#print ("asin: fact = ", fact)
		v = res
		if MPAP_DEGREES_MODE == True:
			#r2d = mpap(180) / mpap('3.1415926535897932384626433832795')
			r2d = mpap(180) / mpap(1).pi()
	...
	def asinalt2 (self, acosine=False):
		global MPAP_DEGREES_MODE
		global MPAPERRORFLAG
		if abs(self) > 1:
			MPAPERRORFLAG = "Domain error."
